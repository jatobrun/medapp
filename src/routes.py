...
def perfil():
    if 'user' in session:
        ...
        if form.validate_on_submit():
            ...
            tabla_paquetes.update_many({'creador': session['user']}, {
                                       '$set': {'creador': form.username.data}})
            tabla_examenes.update_many({'creador': session['user']}, {
                                       '$set': {'creador': form.username.data}})
            tabla_estudios.update_many({'creador': session['user']}, {
                                       '$set': {'creador': form.username.data}})
            tabla_empresas.update_many({'creador': session['user']}, {
                                       '$set': {'creador': form.username.data}})
